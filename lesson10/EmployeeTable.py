...
class EmployeeTable:
    ...
    @allure.step("БД. Получение списка сотрудников компании")
    def get_employees(self):
        return self.db.execute("select * from employee where company_id = 3374").fetchall()
    
    @allure.step("БД. Добавление нового сотрудника")
    ...
    @allure.step("БД. Получение информации о сотруднике по {id}")
    def get_info(self, id):
        sql = text("select * from employee where id =:id_create")
        return self.db.execute(sql, id_create = id)
    
    @allure.step("БД. Получение id добавленного сотрудника")
    def get_max_id(self):
        return self.db.execute("select MAX(id) from employee").fetchall()[0][0]
    
    @allure.step("БД. Изменение информации о сотруднкие: {first_name}, {last_name}, {email}, {company_id}, {phone}, {id}")
    ...
    @allure.step("Удаление сотрудника по {id}")
    def delete(self, id):
